...
public class CTECTwitter
{
	...
	public String analyzeTwitterForTopic(String topic)
	{
		...
		twitterQuery.setGeoCode(new GeoLocation location, double radius, Query.Unit unit);
	...
	public String getMostCommonWord(String username)
	{
		...
		mostCommon = "The common word in " + username + "'s " + searchedTweets.size() + " tweets is " + mostCommonWord + ", and it was used " + maxWord + "times. \nThis is " + (DecimalFormat.getPercentInstance().format(((double) maxWord)/totalWordCount))
				+ " of total words: " + totalWordCount + " and is " + (DecimalFormat.getPercentInstance().format(((double) maxWord)/wordsAndCount.size())) + " of the unique words: " + wordsAndCount.size();
	...
	private ArrayList<Map.Entry<String, Integer>> sort HashMap()
	{
	...
	private void turnStatusesToWords()
	{
		for(Status currentStatus : searchedTweets)
		{
			...
			String [] tweetedWords = tweetText.split(" ");
			for(int index = 0; index < tweetedWords.length; index++)
			{
				tweetedWords.add(removePunctuation(tweetedWords[index]).trim());
			}
		}
